...
Write-Verbose "Entering script SitecorePackageDeployerMonitor.ps1"
...
{
    Write-Verbose "No environment found. Monitoring destination."

    Write-Output (Get-LocalizedString -Key "Monitoring started for - '{0}'" -ArgumentList $targetPath)
    $credential = New-Object 'System.Net.NetworkCredential' -ArgumentList $adminUserName, $adminPassword
    Invoke-Command -ScriptBlock $MonitorSPDJob -ArgumentList "", $sourcePath, $targetPath, $credential, $cleanTargetBeforeCopy, $additionalArguments
}
else
{
    ...
    if($copyFilesInParallel -eq "false" -or  ( $resources.Count -eq 1 ))
    {
        foreach($resource in $resources)
        {
            ...
            Write-Output (Get-LocalizedString -Key "Monitoring started for - '{0}'" -ArgumentList $machine)

            Invoke-Command -ScriptBlock $MonitorSPDJob -ArgumentList $machine, $sourcePath, $targetPath, $resourceProperties.credential, $cleanTargetBeforeCopy, $additionalArguments
        } 
    }
    else
    {
        ...
        foreach($resource in $resources)
        {
            ...
            $job = Start-Job -ScriptBlock $MonitorSPDJob -ArgumentList $machine, $sourcePath, $targetPath, $resourceProperties.credential, $cleanTargetBeforeCopy, $additionalArguments
...
Write-Verbose "Leaving script SitecorePackageDeployerMonitor.ps1"
